...
IMG_NAME=baltimore/openwrt-aarch64-21.4.18

[ -d "$TMPDIR" ] && rm -rf "$TMPDIR"

mkdir -p "$TMPDIR"  && \
gzip -dc openwrt-armvirt-64-default-rootfs-lede.tar.gz | ( cd "$TMPDIR" && tar xf - ) && \
...
rm -rf "$TMPDIR" && \
docker save ${IMG_NAME}:${TAG} | pigz -9 > $OUTDIR/docker-img-openwrt-lede-aarch64-${TAG}.gz
