...
mkdir -p "$TMPDIR"  && \
gzip -dc openwrt-armvirt-64-default-rootfs-official.tar.gz | ( cd "$TMPDIR" && tar xf - ) && \
...
chmod 755 "$TMPDIR/etc/init.d/flowoffload" && \
...
docker build --platform=linux/arm64 -o type=docker -t ${IMG_NAME}:${TAG} . && \
rm -f  openwrt-armvirt-64-default-rootfs.tar && \
rm -rf "$TMPDIR" && \
docker save ${IMG_NAME}:${TAG} | pigz -9 > $OUTDIR/docker-img-openwrt-official-aarch64-${TAG}.gz
